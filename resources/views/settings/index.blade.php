...
@section('css')
    <style>
        .panel-actions .skipper-trash {
            cursor: pointer;
        }

        .panel-actions .skipper-trash:hover {
            color: #e94542;
        }
...
@section('head')
    <script type="text/javascript" src="{{ config('skipper.assets_path') }}/lib/js/jsonarea/jsonarea.min.js"></script>
@stop
...
@section('content')
    ...
    <div class="container-fluid">
        <div class="alert alert-info">
            <strong>如何使用:</strong>
            <p>您可以在网站上任意位置 通过调用 <code>Skipper::setting('key')</code> 获取每个设置的值</p>
        </div>
    </div>

    <div class="page-content container-fluid">
        <form action="{{ route('skipper.settings') }}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="panel">
                @foreach($settings as $setting)
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            {{ $setting->display_name }}<code>Skipper::setting('{{ $setting->key }}')</code>
                        </h3>
                        <div class="panel-actions">
                            <a href="{{ route('skipper.settings.move_up', $setting->id) }}">
                                <i class="sort-icons skipper-sort-asc"></i>
                            </a>
                            <a href="{{ route('skipper.settings.move_down', $setting->id) }}">
                                <i class="sort-icons skipper-sort-desc"></i>
                            </a>
                            <i class="skipper-trash"
                               data-id="{{ $setting->id }}"
                    ...
                    <div class="panel-body">
                        ...
                        @elseif($setting->type == "image" || $setting->type == "file")
                            @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::exists(config('skipper.storage.subfolder') . $setting->value))
                                <div class="img_settings_container">
                                    <a href="{{ route('skipper.settings.delete_value', $setting->id) }}" class="skipper-x"></a>
                                    <img src="{{ Storage::url(config('skipper.storage.subfolder') . $setting->value) }}" style="width:200px; height:auto; padding:2px; border:1px solid #ddd; margin-bottom:10px;">
                                </div>
                    ...
                    <script>
                        $('document').ready(function () {
                            $('#toggle_options').click(function () {
                                ...
                                if ($('#toggle_options .skipper-double-down').length) {
                                    $('#toggle_options .skipper-double-down').removeClass('skipper-double-down').addClass('skipper-double-up');
                                } else {
                                    $('#toggle_options .skipper-double-up').removeClass('skipper-double-up').addClass('skipper-double-down');
                                }
                            });
                        });
                    </script>
    ...
    <script>
        $('document').ready(function () {
            $('.skipper-trash').click(function () {
                var action = '{{ route('skipper.settings') }}/' + $(this).data('id'),
...
@section('javascript')
    ...
    <form id="my_form" action="{{ route('skipper.upload') }}" target="form_target" method="POST" enctype="multipart/form-data" style="width:0;height:0;overflow:hidden">
        {{ csrf_field() }}
        <input name="image" id="upload_file" type="file" onchange="$('#my_form').submit();this.value='';">
        <input type="hidden" name="type_slug" id="type_slug" value="settings">
    </form>

    <script src="{{ config('skipper.assets_path') }}/lib/js/tinymce/tinymce.min.js"></script>
    <script src="{{ config('skipper.assets_path') }}/js/skipper_tinymce.js"></script>
@stop