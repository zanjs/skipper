...
@section('css')
    <link rel="stylesheet" href="{{ config('skipper.assets_path') }}/css/database.css">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="skipper-data"></i> Database
        <a href="{{ route('skipper.database.create_table') }}" class="btn btn-success"><i class="skipper-plus"></i>
            Create New Table</a>
    </h1>
@stop
    ...
    <div class="page-content container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                <?php $dataTypes = Anla\Skipper\Models\DataType::all(); ?>
                <?php $dataTypeNames = []; ?>
                ...
                <?php $arr = Anla\Skipper\Facades\DBSchema::tables(); ?>
                @foreach($arr as $a)
	            <?php $table = current($a); ?>
                	<?php $active = in_array($table, $dataTypeNames); 
                        if($active){
                            $activeDataType = Anla\Skipper\Models\DataType::where('name', '=', $table)->first();
                        }
                    ?>
                        ...
                        <tr>
                            <td>
                                <p class="name">
                                    @if($active)
                                        <a href="{{ route('skipper.database.browse_table', $table) }}"
                            ...
                            <td class="actions">
                                <a class="btn-danger btn-sm pull-right delete_table @if($active) remove-bread-warning @endif"
                                   data-table="{{ $table }}" style="display:inline; cursor:pointer;"><i
                                            class="skipper-trash"></i> Delete</a>
                                <a class="btn-sm btn-primary pull-right" style="display:inline; margin-right:10px;"
                                   href="{{ route('skipper.database.edit_table', $table) }}"><i
                                            class="skipper-edit"></i> Edit</a>
                                <a class="btn-sm btn-warning pull-right desctable"
                                   style="display:inline; margin-right:10px;"
                                   href="{{ route('skipper.database.browse_table', $table) }}" data-name="{{ $table }}"><i
                                            class="skipper-eye"></i> View</a>
                            </td>
                        </tr>
                    @endforeach
                </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="skipper-trash"></i> Are you sure you want to delete the BREAD for
                        the <span id="delete_builder_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('skipper.database') }}/delete_bread" id="delete_builder_form" method="POST">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="skipper-trash"></i> Are you sure you want to delete the <span
                                id="delete_table_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('skipper.database') }}/table/delete" id="delete_table_form" method="POST">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="skipper-data"></i> @{{ table.name }}</h4>
                </div>
