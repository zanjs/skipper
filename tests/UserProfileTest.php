...
class UserProfileTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->editPageForTheCurrentUser = config('skipper.routes.prefix')."/users/{$this->user->id}/edit";

        $this->listOfUsers = config('skipper.routes.prefix').'/users';
    }

    public function testCanSeeTheUserInfoOnHisProfilePage()
    {
        $this->visit(route('skipper.profile'))
    ...
    public function testCanEditUserPassword()
    {
        $this->visit(route('skipper.profile'))
             ...
             ->see('Edit User')
             ->seePageIs($this->editPageForTheCurrentUser)
             ->type('skipper-rocks', 'password')
             ->press('Submit')
             ->seePageIs($this->listOfUsers);

        $updatedPassword = DB::table('users')->where('id', 1)->first()->password;
        $this->assertTrue(Hash::check('skipper-rocks', $updatedPassword));
    }
