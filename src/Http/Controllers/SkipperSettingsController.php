...
class SkipperSettingsController extends Controller
{
    public function index()
    {
        $settings = Setting::orderBy('order', 'ASC')->get();

        return view('skipper::settings.index', compact('settings'));
    }
    ...
    public function delete_value($id)
    {
        ...
        if (isset($setting->id)) {
            ...
            if ($setting->type == 'image') {
                if (Storage::exists(config('skipper.storage.subfolder').$setting->value)) {
                    Storage::delete(config('skipper.storage.subfolder').$setting->value);
                }
            }
            $setting->value = '';
            $setting->save();
    ...
    public function save(Request $request)
    {
        ...
        $breadController = new SkipperBreadController(); // TODO: This is bad!! Extract getContentBasedOnType() as a Helper
